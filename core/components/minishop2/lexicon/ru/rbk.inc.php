<?php
/**
 * Settings Russian Lexicon Entries for miniShop2-RBK
 *
 * @package minishop2-rbk
 * @subpackage lexicon
 */

$_lang['ms2_payment_rbk_service_name'] = 'Заказ #[[+num]]';
$_lang['setting_ms2_payment_rbk_checkout_url'] = 'Url оплаты RBK Money';
$_lang['setting_ms2_payment_rbk_currency'] = 'Валюта на RBK Money';
$_lang['setting_ms2_payment_rbk_currency_desc'] = 'USD, RUR, EUR, UAH, GBP';
$_lang['setting_ms2_payment_rbk_eshopid'] = 'Идентификатор магазина в RBK Money';
$_lang['setting_ms2_payment_rbk_secret_key'] = 'Секретрый ключ RBK Money';
$_lang['setting_ms2_payment_rbk_language'] = 'Язык интерфейса RBK Money';
$_lang['setting_ms2_payment_rbk_language_desc'] = 'ru или en';
$_lang['setting_ms2_payment_rbk_preference'] = 'Способ пополнения в RBK Money';
$_lang['setting_ms2_payment_rbk_preference_desc'] = 'bankcard для кредитных карт, postrus для почты России итд. Или пустое для выбора пользователем';
$_lang['setting_ms2_payment_rbk_success_id'] = 'Страница успешной оплаты RBK Money';
$_lang['setting_ms2_payment_rbk_cancel_id'] = 'Страница отказа от оплаты RBK Money';
$_lang['setting_ms2_payment_rbk_cancel_order'] = 'Отмена заказа RBK Money';
$_lang['setting_ms2_payment_rbk_cancel_order_desc'] = 'Если включено, заказ будет отменён при отказе от оплааты.';